"""
Multi-Step Auto Clicker - Hỗ trợ Copy mã DIETCOKE & Hotkey F11/F12
"""
...
class App:
    def __init__(self, root):
        ...
        ttk.Button(footer_frame, text="💾 Lưu cấu hình", command=self.save_config).pack(side="left", padx=4)
        ttk.Button(footer_frame, text="📂 Tải cấu hình", command=self.load_config).pack(side="left", padx=4)
    ...
    def start_hotkey_listener(self):
        """Khởi chạy pynput để lắng nghe phím F11 và F12 trên toàn hệ thống"""
        def on_press(key):
            try:
    ...
    def save_config(self):
        ...
        try:
            ...
            self.msg_queue.put(f"Đã lưu {len(self.steps)} bước vào {CONFIG_FILE}.")
        except Exception as e:
    ...
    def load_config(self, silent=False):
        ...
        try:
            ...
            self.refresh_listbox()
            self.msg_queue.put(f"Đã tải {len(self.steps)} bước từ {CONFIG_FILE}.")
        except Exception as e:
            if not silent:
